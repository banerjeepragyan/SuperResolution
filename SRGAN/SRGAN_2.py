...
if not os.path.exists(SAVEPRE_DIR):
    os.makedirs(SAVEPRE_DIR)
...
def main():
    ...
    g_pre = tf.train.AdamOptimizer(1e-4,beta1=0.5).minimize(pre_loss, var_list=[
            x for x in tf.trainable_variables() if "SRGAN_g"     in x.name])
    ...
    d_opt = tf.train.AdamOptimizer(lr/2,beta1=0.5).minimize(d_loss, var_list=[
            x for x in tf.trainable_variables() if "SRGAN_d" in x.name])
    ...
    for p in range(50001):
        ...
        tmp, gen_loss,L1,vgg = sess.run([g_pre,pre_loss,L1_loss,vgg_loss], feed_dict={
            x: batch_images_x,
            t: batch_images_t
        })

        hist.append(gen_loss)
        print("in step %s, pre_loss =%.4e, L1_loss=%.4e, vgg_loss=%.4e" %(p, gen_loss, L1, vgg))
    ...
    for i in range(100001):
        ...
        tmp, gen_loss, L1, adv, vgg, = sess.run([g_opt,g_loss,L1_loss,g_loss_fake,vgg_loss], feed_dict={
            x: batch_images_x,
            t: batch_images_t,
            lr:trans_lr,
        })
