...
def main(_):
    ...
    for lr, hr in train_dataset.take(remain_steps):
        ...
        if steps % cfg['save_steps'] == 0 or steps == 1:
            manager.save()
        ...
        if steps % cfg['save_steps'] == 0 or steps == 1:
            #load checkpoint
            model_valid = RRDB_Model(None, cfg['ch_size'], cfg['network_G'])
            checkpoint_valid_dir = './checkpoints/' + cfg['sub_name']
            checkpoint_valid = tf.train.Checkpoint(model=model_valid)
            checkpoint_valid.restore(tf.train.latest_checkpoint(checkpoint_valid_dir))
            #evaluate valid
            psnr_val = []
            ssim_val = []
            for key, path in cfg['valid_dataset'].items():
                for img_name in os.listdir(path):
                    raw_img = cv2.imread(os.path.join(path, img_name))
                    lr_img, hr_img = create_lr_hr_pair(raw_img, cfg['scale'])
                    sr_img = tensor2img(model_valid(lr_img[np.newaxis, :] / 255))
                    psnr_val.insert(len(psnr_val), calculate_psnr(rgb2ycbcr(sr_img), rgb2ycbcr(hr_img)))
                    ssim_val.insert(len(ssim_val), calculate_ssim(rgb2ycbcr(sr_img), rgb2ycbcr(hr_img)))
            mean_psnr_val = np.mean(psnr_val)
            mean_ssim_val = np.mean(ssim_val)
            final_result = "   MEAN PSNR = " + str(mean_psnr_val) + "   :::   MEAN SSIM = " + str(mean_ssim_val)
            print (final_result)  
            #END OF VALIDATION
